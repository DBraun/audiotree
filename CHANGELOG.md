...
* **String leaf support in `TreeWriter` / `TreeDataSource`**: `write()` now accepts `str` or `List[str]` leaves alongside arrays and AudioTrees. Strings are stored in [Bagz](https://github.com/google/bagz) files (one per string leaf) with no length limits or truncation. Single-sample reads return a bare `str`, and `AudioTree.batch` naturally collects them into `List[str]`. Requires the optional `bagz` dependency (`pip install bagz`); array-only trees work without it.
