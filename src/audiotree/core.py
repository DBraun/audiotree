...
class ExcerptConfig:
    ...
    ``"random"``
        A random offset drawn by ``search`` -- uniform by default (the
        default strategy). No audio is measured, so this costs one read per
        item.
    ``"start"``
    ...
    Attributes:
        ...
        lufs_cutoff: Integrated loudness (LUFS) that ends the search early
            (``"loudest"`` only).
        search: How each offset is drawn (``"random"`` and ``"loudest"``; not
            meaningful under ``"start"``) — a registered name (``"uniform"``,
            ``"bias_early"``), a dotted path to an importable function, or a
            callable.
    ...
    #: ``search`` is not here: it also drives the single draw of
    #: ``strategy="random"`` (see :meth:`AudioTree.excerpt`), and is only
    #: rejected under ``"start"``, which draws nothing.
    _LOUDEST_ONLY = ("num_tries", "lufs_cutoff", "on_failure")
    ...
    def __post_init__(self):
        ...
        if self.strategy == "start" and self.search != "uniform":
            raise ValueError(
                f"search draws random offsets, but strategy='start' always "
                f"reads from the beginning; got search={self.search!r}."
            )
...
def _check_extras_leaves(extras: dict, path: str = "extras") -> None:
    """Reject Python lists/tuples of non-strings as ``extras`` leaves.

    A list of *strings* is a supported leaf, treated as one value per batch
    item. A list of numbers looks like it should behave the same, but
    ``jax.tree_util`` descends into it and treats every number as its own
    scalar leaf: batch-axis indexing and :meth:`AudioTree.filter` would pass
    the whole list through at full length, silently misaligning it with the
    batch. Rejecting it at :meth:`AudioTree.create` turns that silent
    corruption into an immediate error naming the fix.
    """
    if _is_string_leaf(extras):
        return
    if isinstance(extras, dict):
        for key, value in extras.items():
            _check_extras_leaves(value, f"{path}[{key!r}]")
        return
    if isinstance(extras, (list, tuple)):
        raise TypeError(
            f"{path} is a {type(extras).__name__} of non-string values, which "
            f"does not batch or index per item (each element would become its "
            f"own scalar leaf). Convert it with np.asarray(...); lists of "
            f"strings are the only supported list leaves."
        )
...
class AudioTree:
    ...
    Args:
        ...
        extras (dict): Any extra per-item data can be placed here. This dict is entirely yours:
            the library never plants keys of its own in it. Leaves should be arrays with the
            batch as their leading axis, or lists of strings (one per item) -- :meth:`create`
            validates this; a list of numbers would be flattened into scalar leaves by
            ``jax.tree_util`` and silently stop tracking the batch.
        _metadata (dict): Library-managed provenance container -- private, as the underscore
            says; not for user data (put your own data in ``extras``). It holds the encoded
            ``"filepath"`` and ``"source"`` arrays plus the per-item ``"offset"`` array: pass
            ``filepath=`` / ``source=`` / ``offset=`` to :meth:`create` to fill it, and read
            the values back via the :attr:`filepath`, :attr:`source` and :attr:`offset`
            properties. On disk it keeps the spelling ``metadata``.
    ...
    # Leading underscore: this container is library-internal. Read provenance
    # through the ``.filepath``/``.source``/``.offset`` properties and write it
    ...
    def create(
        ...
        *,
        ...
        offset: Union[float, ArrayLike] | None = None,
    ) -> Self:
        ...
        you don't have to reshape by hand. ``filepath``, ``source`` and ``offset`` go into the
        library-managed ``_metadata`` container; ``extras`` is left to the caller.
        ...
        Args:
            ...
            extras: Optional extras dict of additional per-item leaves (copied, not
                mutated). Leaves must be arrays with the batch as their leading axis,
                lists of strings (one per item), or nested dicts of those; a Python
                list/tuple of non-strings is rejected -- ``jax.tree_util`` would
                treat each element as its own scalar leaf, silently misaligning it
                with the batch. Convert such lists with ``np.asarray``.
            filepath: Optional path(s) for the batch; encoded into ``_metadata["filepath"]`` and read
            ...
            source: Optional source-group name(s) (e.g. ``"music"``), encoded into
                ``_metadata["source"]`` and read back via the :attr:`source` property. Pass a
            ...
            offset: Optional source-file offset(s) in seconds, stored in
                ``_metadata["offset"]`` and read back via the :attr:`offset` property. Pass a
                single number to tag the whole batch, or a ``(Batch,)`` array with one offset
                per batch item. :meth:`from_file` and the dataset loaders record this
                automatically.
        ...
        if offset is not None:
            if np.ndim(offset) == 0:
                # A single number tags the whole batch, mirroring filepath/source.
                n = batch_size if batch_size is not None else 1
                offset_array = np.full((n,), float(offset), dtype=np.float64)
            else:
                offset_array = np.asarray(offset, dtype=np.float64)
                if offset_array.ndim != 1:
                    raise ValueError(
                        f"AudioTree.create expects offset as a single number or a "
                        f"(Batch,) array, got rank {offset_array.ndim} "
                        f"{tuple(offset_array.shape)}."
                    )
                if batch_size is not None and len(offset_array) != batch_size:
                    raise ValueError(
                        f"AudioTree.create got {len(offset_array)} offsets for a "
                        f"batch of {batch_size}. Pass a single number to tag the "
                        f"whole batch, or an array with one offset per batch item."
                    )
            metadata["offset"] = offset_array
    ...
    def _invalidate_derived(
        ...
        ``lufs``, ``lufs_windows``, ``codes`` and ``latents``
        (:data:`DERIVED_FIELDS`) all describe one specific waveform; every
        ...
        """
        keep = set(keep)
        unknown = sorted(keep - set(DERIVED_FIELDS))
        if unknown:
            raise ValueError(
    ...
    def replace_lufs(
        ...
        *Where* the measurement runs and *which* kernel runs are separate
        ...
        if self.waveform is None:
            raise ValueError(
                "AudioTree.replace_lufs needs a waveform to measure, but this "
                "tree has none (a token-only tree holds codes/latents; decode "
                "them first)."
            )
    ...
    def clear_lufs(self) -> Self:
        """Return a copy with the cached ``lufs`` and ``lufs_windows`` dropped.

        Shorthand for ``replace(lufs=None, lufs_windows=None)``. Call it after
        changing the waveform through a bare :meth:`replace`, so the next
        :meth:`replace_lufs` measures the new audio instead of trusting a
        cached value that describes the old one. The built-in transforms and
        AudioTree methods invalidate for you; this is for hand-rolled edits.
        ...
        Example:
            ...
            True
        """
        return self.replace(lufs=None, lufs_windows=None)
    ...
    def normalize_lufs(
        ...
        *,
        ...
        ``codes`` and ``latents``, which describe the
        ...
        if self.waveform is None:
            raise ValueError(
                "AudioTree.normalize_lufs needs a waveform to scale, but this "
                "tree has none (a token-only tree holds codes/latents; decode "
                "them first)."
            )
    ...
    def filepath(self) -> Union[List[str], List[list]]:
        ...
        provenance.
        """
        if "filepath" not in self._metadata:
            return []
        return self._decode_strings(self._metadata["filepath"], "filepath")
    ...
    def source(self) -> Union[List[str], List[list]]:
        ...
        For example, if :func:`~audiotree.sources.create_balanced_audio_dataset`
        was given ``sources={"music": [...], "speech": [...]}``, this property
        might return ``["music", "music", "speech", "music"]`` for a batch of
        4 items. Like :attr:`filepath`, a mini-batched (rank-4) tree nests one
        list per mini-batch.
        ...
        if "source" not in self._metadata:
            return []
        return self._decode_strings(self._metadata["source"], "source")

    @property
    def offset(self) -> Optional[ArrayLike]:
        """Return the per-item source-file offsets stored in ``_metadata['offset']``.

        Seconds from the start of each item's source file to its sample 0,
        recorded by :meth:`from_file` and the dataset loaders. The shape follows
        the tree's leading axes: ``(Batch,)`` normally, ``(Mini, Batch)`` on a
        mini-batched tree. Returns ``None`` when the tree carries no offset
        provenance, or when a time-shifting transform such as
        :func:`~audiotree.transforms.roll` invalidated it (under ``prob < 1``
        the whole batch's offset is dropped, since "invalidated for some items
        only" cannot be represented in one array). An item with no source file
        (e.g. a synthetic pre-built dataset group) records ``NaN``.
        """
        return self._metadata.get("offset")
    ...
    def __getitem__(
        ...
        elif not isinstance(key, (slice, np.ndarray, jax.Array)):
            # Normalize a Python list/tuple key to an array once, here: NumPy
            # leaves accept a bare list but JAX leaves reject it, so the same
            # tree would index differently depending on its backend. String
            # leaves already normalize inside ``_index_string_list``.
            key = np.asarray(key)
            if key.size == 0:
                # An empty list has no dtype of its own and defaults to
                # float64, which is not a valid index type; an empty
                # integer key selects the empty sub-batch.
    ...
    def from_file(
        ...
        *,
        ...
        Args:
            ...
            filepath (Union[str, Path, List[str | Path]], optional): The path to store as the returned
                ``AudioTree``'s provenance (``_metadata["filepath"]``, read back via the ``filepath``
                property) -- a single path or a one-item list, since ``from_file`` returns a batch
                of 1. If *None* (default) the provided ``audio_path`` will be used.
            source (str, optional): The source group name for this audio file (e.g., "music", "speech").
                Stored as provenance (``_metadata["source"]``) and accessible via the ``source`` property.
            extras (dict, optional): Additional extras to include in the AudioTree. The dict is
                purely user payload; the read offset is recorded as provenance
                (``_metadata["offset"]``, read back via the :attr:`offset` property).
        ...
        if offset > 0 and data.shape[-1] == 0:
            # An offset at or past the end of the file reads nothing; padding
            # it out would return full-duration silence stamped with an offset
            # that points at audio which does not exist. The excerpt searches
            # clamp their offsets to the file duration, so reaching this means
            # the caller computed the offset itself (an off-by-one windowing
            # scheme, or samples passed where seconds were expected).
            raise ValueError(
                f"offset={offset}s is at or past the end of {audio_path} "
                f"({soundfile.info(audio_path).duration:.3f}s): the read is "
                f"empty. Offsets are in seconds from the start of the file."
            )
        ...
        # Provenance goes into the library-managed ``_metadata`` container, so
        # the user's ``extras`` keys -- including one literally named
        # "filepath" or "offset" -- are never displaced.
        if filepath is None:
            ...
            if len(paths_to_store) != 1:
                # Metadata rows align one per batch item and ``from_file``
                # always returns a batch of 1, so a longer list would silently
                # misalign provenance -- and ``AudioTree.batch`` would then
                # attribute rows to the wrong items. Same guard ``create``
                # applies.
                raise ValueError(
                    f"AudioTree.from_file returns a batch of 1, but got "
                    f"{len(paths_to_store)} filepath entries. Pass a single "
                    f"path (or use AudioTree.create for pre-batched arrays)."
                )
        ...
        # Record where the excerpt starts in the source file, read back via the
        # ``offset`` property.
        metadata["offset"] = np.array([offset], dtype=np.float64)
    ...
    def from_manifest(
        ...
        *,
        ...
        # ``source=`` re-encodes them into the ``_metadata`` container and makes
        ...
        offsets = stack_column("offset")
        if offsets is not None:
            tree_kwargs["offset"] = offsets.astype(np.float64)
    ...
    def to_mono(
        ...
        is invalidated. A waveform that is already
    ...
    def resample(
        ...
        *,
        ...
        is invalidated. Resampling to the rate the
...
#: and ``_metadata``) are excluded.
ARRAY_FIELDS: tuple = tuple(
    f for f in PYTREE_FIELDS if f not in ("extras", "_metadata")
)
