...
from .core import LABEL_FIELDS, AudioTree, _require_single_batch
...
_SKIPPED_EXTRAS_KEYS = frozenset({"duration", "manifest_index", "tags"})
...
class AudioWriter:
    ...
    Example:
        ...
        >>> pbar = tqdm(total=len(audio_batches), desc="Processing")  # doctest: +SKIP
    ...
    def write(self, tree: AudioTree, tags: Optional[Dict] = None) -> List[Path]:
        ...
        _require_single_batch(tree, "AudioWriter.write")
        ...
        # Validate field consistency. Schema expectations from a first write
        # are *recorded* only in the commit step at the end of this method, so
        # a first write() that fails leaves the writer exactly as it was and a
        # corrected retry is not checked against the failed call's schema.
        present_fields = self._get_present_fields(tree)
        if self._expected_fields is not None:
        ...
        provenance["offset"] = tree.offset is not None
        if self._expected_extras_keys is not None:
        ...
        # Same ragged-column failure for the offset array (reachable only via
        # hand-assembled metadata -- ``create`` validates the length).
        offsets = tree.offset
        if offsets is not None and len(offsets) < batch_size:
            raise ValueError(
                f"AudioTree has {len(offsets)} offsets for a batch of "
                f"{batch_size}; the 'offset' column would cover only part of "
                f"this write. Provide one offset per item, or none."
            )
        ...
        # Phase 1: collect and validate every entry of the batch with nothing
        # on disk and no writer state touched. A column this writer cannot
        # store -- or a label array shorter than the batch -- fails here, at
        # its own write() call, keeping the directory free of audio no
        # manifest row points at. Kinds are checked against a scratch copy so
        # a rejected batch does not pin column kinds either.
        entries: List[Dict] = []
        ...
        for i in range(batch_size):
            ...
            filename = self.pattern.format(index=self.index + i)
        ...
        # Phase 2: write the audio. Failing on item k used to leave items
        # 0..k-1 on disk, in the manifest, and counted in self.index -- so the
        # natural recovery, fixing the tree and calling write() again,
        # silently duplicated them. Now a mid-batch failure deletes the WAVs
        # this call already wrote (no manifest row points at them) and
        # re-raises with the writer state untouched, so a retry starts clean.
        written: List[Path] = []
        if self.write_audio:
            filepath = None
            try:
                for i, filepath in enumerate(paths):
                    ...
                    # A fixed-point subtype silently hard-clips out-of-range
                    # samples, so measure the peak before handing the audio to
                    # libsndfile and report it rather than let the data
                    # disappear.
                    if subtypes[i] not in _FLOATING_SUBTYPES and audio.size:
                        peak = float(np.abs(audio).max())
                        if peak > 1.0:
                            clipped.append((paths[i].name, peak, subtypes[i]))
                    soundfile.write(
                        str(filepath), audio, tree.sample_rate, subtype=subtypes[i]
                    )
                    written.append(filepath)
            except BaseException:
                # Includes the file the failing soundfile.write may have
                # partially created.
                cleanup = list(written)
                if filepath is not None and filepath not in written:
                    cleanup.append(filepath)
                for stale_path in cleanup:
                    try:
        ...
        # Phase 3: commit, all together or not at all.
        self.written_paths.extend(written)
    ...
    def _create_manifest_entry(
        ...
        *,
        ...
        Args:
            ...
            index: The running dataset index for this item. Passed in rather
                than read from ``self.index``, which advances only when the
                whole batch commits.
        ...
        # The recorded source-file offset is provenance too, one float per item.
        offsets = tree.offset
        if offsets is not None and batch_index < len(offsets):
            entry["offset"] = float(offsets[batch_index])
    ...
    def _check_entry_kinds(
        ...
        Args:
            ...
            column_kinds: The established kind per column, updated in place.
