...
from audiotree.core import DERIVED_FIELDS, LABEL_FIELDS
...
class AudioDataSource(grain.RandomAccessDataSource):
    ...
    Args:
        path: The dataset directory an :class:`~audiotree.writer.AudioWriter`
            produced (its ``manifest.npz`` is located inside), or an explicit
            path to a manifest file (NPZ format)
        ...
        duration: Optional duration to trim/pad audio to (in seconds)
            ...
            When ``sample_rate``, ``mono`` or ``duration`` change the audio
            relative to what was written, the manifest's derived fields
            (``lufs``, ``lufs_windows``, ``codes``, ``latents``) are *not*
            restored -- they describe the written audio, not the altered one.
            Annotation fields (``pitch``, ``velocity``, ``note_duration``) are
            restored either way.
        ...
        on_read_error: What to do when one entry's audio file is missing or
            ...
            ``"warn"`` drops the item (``ds[i]`` returns ``None``, which grain
            skips at iteration) and emits a :class:`UserWarning`; ``"skip"``
            drops it quietly. See
    ...
    Example:
        ...
        Or point straight at the dataset directory (its ``manifest.npz`` is
        located inside), the same way :class:`TreeDataSource` is used:
    ...
    def __init__(
        ...
        *,
        ...
        # A dataset directory works the same way TreeDataSource is pointed at
        # one; an explicit manifest file path is also accepted.
        path = Path(path)
        self.manifest_path = path / "manifest.npz" if path.is_dir() else path
    ...
    def _load_alters_audio(self, entry: Dict) -> bool:
        """Whether this source's load parameters change ``entry``'s audio.

        The manifest's derived fields (``lufs``, ``lufs_windows``, ``codes``,
        ``latents``) describe the audio *as written*. When ``sample_rate``,
        ``mono`` or ``duration`` make :meth:`AudioTree.from_file` hand back
        different audio, restoring those fields would attach measurements of
        audio the caller is not getting -- silently, with nothing downstream
        able to detect it. Label fields (``pitch``, ...) are annotations, not
        measurements, and are restored either way.
        """
        original_sr = entry.get("sample_rate")
        if (
            self.sample_rate is not None
            and original_sr is not None
            and self.sample_rate != original_sr
        ):
            return True
        if self.mono and int(entry.get("channels", 1)) > 1:
            return True
        if self.duration is not None:
            samples = entry.get("samples")
            if samples is None or original_sr is None:
                # Cannot prove the requested duration is a no-op.
                return True
            if round(self.duration * original_sr) != int(samples):
                return True
        return False
    ...
    def __getitem__(self, record_key: SupportsIndex) -> Optional[AudioTree]:
        ...
        Returns:
            AudioTree with audio data and restored extras, or ``None`` when the
            entry's audio file cannot be read under a non-raising
            ``on_read_error`` policy (grain skips ``None`` elements at
            iteration).
        ...
        if files_written:
            ...
            # along the wrong axis by ``AudioTree.batch``. Derived fields are
            # restored only when the load parameters leave the audio as
            # written (see ``_load_alters_audio``); left unset, the next
            # ``replace_lufs()`` measures the audio actually returned.
            alters_audio = self._load_alters_audio(entry)
            for field_name in LABEL_FIELDS:
                if field_name not in entry:
                    continue
                if alters_audio and field_name in DERIVED_FIELDS:
                    continue
            ...
            try:
                ...
                if self.on_read_error == "warn":
                    warnings.warn(
                        f"Skipping unreadable audio file {str(audio_path)!r} "
                        f"for manifest entry {int(record_key)}: "
                        f"{type(exc).__name__}: {exc}. The item is dropped "
                        "(grain skips None elements at iteration); pass "
            ...
            if source_group is not None:
                tree_kwargs["source"] = source_group
            tree_kwargs["offset"] = float(entry.get("offset", 0.0))
        ...
        # Restore the offset recorded at write time (where the item's excerpt
        # started in its original source file). ``from_file`` stamps 0.0 -- the
        # start of the written WAV -- which is right only when the manifest
        # recorded nothing.
        if "offset" in entry:
            audio_tree = audio_tree.replace(
                _metadata={
                    **audio_tree._metadata,
                    "offset": np.array([float(entry["offset"])], dtype=np.float64),
                }
            )
