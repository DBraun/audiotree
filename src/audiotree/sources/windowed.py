...
from audiotree.loudness import _window_samples
...
def _file_windows(
    ...
    # Round rather than truncate (via the shared helper), so the analysis grid
    # here agrees with ``replace_lufs`` and with the seconds-based cell mapping
    # in ``_build_slot_index``: ``int(0.7 * 44100)`` is 30869, one sample short
    # of the true window, drifting the grid one sample per window.
    window_samples = _window_samples(lufs_window_sec, sr)
...
class WindowLufsCache:
    ...
    Attributes:
        ...
        file_sizes: Mapping from filepath to its byte size when the cache was
            built, or ``None`` for caches written before sizes were recorded.
            Used to detect a file re-rendered in place, whose cached durations
            and loudness would otherwise silently describe audio that no
            longer exists. (A rewrite that keeps the byte size identical is
...
def save_window_lufs(
    ...
    *,
    ...
    the bagz records), the analysis window, optional durations, the
    ``sample_rate``/``mono`` the loudness was measured at (so the dataset can
    verify the cache matches the audio it loads), and each file's byte size,
    so a file re-rendered in place after the build is refused at dataset
    construction rather than silently keeping stale durations and loudness.
    A rewrite that keeps the byte size identical is not detectable this way
    (see :class:`WindowLufsCache`).
    ...
    # Record each file's byte size so a file re-rendered in place after the
    # cache was built names itself at load time instead of silently keeping
    # the old durations and loudness. A file that vanished between analysis
    # and here gets no size; the dataset's slot builder reports it anyway.
    file_sizes = []
    for fp in filepaths:
        try:
...
def load_window_lufs(cache_dir: str | Path) -> WindowLufsCache:
    ...
    def _parallel_list(key: str) -> Optional[list]:
        """A manifest list parallel to ``filepaths``, length-checked.

        ``zip`` truncates silently, so a hand-edited or corrupted manifest
        whose list is shorter than ``filepaths`` would quietly drop the tail
        files' values -- exactly the corruption class the checks above exist
        to name.
        """
        values = manifest.get(key)
        if values is None:
            return None
        if not isinstance(values, list) or len(values) != len(filepaths):
            count = len(values) if isinstance(values, list) else values
            raise ValueError(
                f"Invalid manifest {manifest_path}: {key!r} has {count!r} "
                f"entries for {len(filepaths)} filepaths."
            )
        return values

    durations = _parallel_list("durations")
    if durations is not None:
        durations = dict(zip(filepaths, durations))

    # Caches written before sizes were recorded have no "file_sizes" key.
    file_sizes = _parallel_list("file_sizes")
    if file_sizes is not None:
        file_sizes = {
            fp: size for fp, size in zip(filepaths, file_sizes) if size is not None
        }
...
def _build_slot_index(
    ...
    for i, fp in enumerate(filepaths):
        if fp not in durations:
            raise ValueError(
                f"No duration recorded for {fp!r}: the durations mapping (from "
                f"a lufs_cache or an explicit durations= argument) predates "
                f"this file. Rebuild the cache with build_window_lufs_cache "
                f"over the current file list, or pass durations=None to "
                f"rescan the file headers."
            )
        ...
        if lufs_per_file is not None:
            if fp not in lufs_per_file:
                # Fail loud, and say what happened: a bare KeyError names only
                # the path, not the stale cache or the remedy.
                raise ValueError(
                    f"No windowed-LUFS entry for {fp!r}: the loudness cache "
                    f"predates this file. Rebuild it with "
                    f"build_window_lufs_cache over the current file list."
                )
            lufs = lufs_per_file[fp]
            if lufs.size > 0:
...
def create_windowed_audio_dataset(
    ...
    *,
    ...
    if lufs_cache is not None:
        ...
        # A file re-rendered in place after the cache was built would silently
        # keep the cached durations and loudness -- slots and offsets computed
        # for audio that no longer exists. Byte sizes were recorded at build
        # time, so a changed file names itself here. (Missing files, and files
        # added after the build, are reported by the slot builder below.)
        if cache.file_sizes is not None:
            stale = []
            for fp in filepaths:
                expected_size = cache.file_sizes.get(fp)
                if expected_size is None:
                    continue
                try:
                    if os.path.getsize(fp) != expected_size:
                        stale.append(fp)
                except OSError:
                    continue
            if stale:
                shown = ", ".join(repr(fp) for fp in stale[:3])
                more = f" (and {len(stale) - 3} more)" if len(stale) > 3 else ""
                raise ValueError(
                    f"lufs_cache is stale: {shown}{more} changed on disk since "
                    f"the cache was built. Rebuild it with "
                    f"build_window_lufs_cache."
                )
