...
def _check_hop_factor(name: str, hop_factor: float, frame_length: int) -> None:
    """Reject a hop size the STFT round-trip cannot reconstruct from.

    Above ``0.5`` the hann analysis windows no longer overlap-add to a constant:
    at ``hop_factor=1.0`` even ``amount=0.0`` (mathematically the identity)
    corrupts chunks of every frame, and between ``0.5`` and ``1.0`` the NumPy
    and JAX backends silently diverge on the tail samples. A ``hop_factor``
    small enough that ``int(frame_length * hop_factor)`` truncates to zero
    fails too, with a librosa ``ParameterError`` on one backend and a bare
    ``ZeroDivisionError`` on the other. Validating here keeps both backends
    consistent with a single clear error, like the other ``_check_*`` guards.
    """
    if not 0 < hop_factor <= 0.5:
        raise ValueError(
            f"{name} requires 0 < hop_factor <= 0.5, but got "
            f"hop_factor={hop_factor}. Above 0.5 the analysis windows no "
            f"longer overlap enough to reconstruct the signal, so even "
            f"amount=0.0 would corrupt the audio."
        )
    if int(frame_length * hop_factor) < 1:
        raise ValueError(
            f"{name} got hop_factor={hop_factor} with "
            f"frame_length={frame_length}, which truncates to a hop of zero "
            f"samples. Use a larger hop_factor or frame_length."
        )
...
def _volume_norm_jax(
    ...
    # Route through ``_invalidate_derived`` so the level change also drops any
    # cached ``codes``/``latents``; the shifted loudness is supplied fresh.
...
def _volume_norm_np(
    ...
    # See ``_volume_norm_jax``: also drops any cached ``codes``/``latents``.
    return audio_tree._invalidate_derived(
        waveform=waveform,
...
def _volume_change_jax(
    ...
    waveform = waveform * _db2linear_jax(gain_db)[:, None, None]
    # ``lufs``/``lufs_windows`` are kept because the caller shifts them by the
    # returned ``gain_db``; ``codes``/``latents`` describe the old level.
    return (
        audio_tree._invalidate_derived(
            keep=("lufs", "lufs_windows"), waveform=waveform
        ),
        gain_db,
    )
...
def _volume_change_np(
    ...
    waveform = waveform * _db2linear_np(gain_db)[:, None, None]
    # See ``_volume_change_jax``: lufs is kept for the caller's shift.
    return (
        audio_tree._invalidate_derived(
            keep=("lufs", "lufs_windows"), waveform=waveform
        ),
...
def _rescale_audio_jax(audio_tree: AudioTree) -> AudioTree:
    ...
    # A level change: every derived field (lufs and codes/latents) is stale.
    return audio_tree._invalidate_derived(waveform=waveform)
...
def _invert_phase_jax(audio_tree: AudioTree) -> AudioTree:
    ...
    waveform = -audio_tree.waveform
    # Negation preserves energy exactly, so the cached loudness stays valid;
    # ``codes``/``latents`` describe the un-negated waveform and are dropped.
    return audio_tree._invalidate_derived(
        keep=("lufs", "lufs_windows"), waveform=waveform
    )
...
def _invert_phase_np(audio_tree: AudioTree) -> AudioTree:
    ...
    waveform = -audio_tree.waveform
    # See ``_invert_phase_jax``: loudness is kept, codes/latents are dropped.
    return audio_tree._invalidate_derived(
        keep=("lufs", "lufs_windows"), waveform=waveform
    )
...
def _swap_stereo_jax(audio_tree: AudioTree) -> AudioTree:
    ...
    # BS.1770 weights left and right equally, so the cached loudness stays
    # valid; ``codes``/``latents`` describe the un-swapped channel order.
    return audio_tree._invalidate_derived(
        keep=("lufs", "lufs_windows"), waveform=waveform
    )
...
def _swap_stereo_np(audio_tree: AudioTree) -> AudioTree:
    ...
    # See ``_swap_stereo_jax``: loudness is kept, codes/latents are dropped.
    return audio_tree._invalidate_derived(
        keep=("lufs", "lufs_windows"), waveform=waveform
    )
...
def _pad_to_hop_multiple(waveform: jnp.ndarray, hop_length: int) -> jnp.ndarray:
    ...
    off. ``librosa.istft`` reconstructs the tail itself at the hop sizes
    ``_check_hop_factor`` admits (hop <= frame_length / 2), so the NumPy
    backend needs no such padding.
    """
...
def _corrupt_phase_jax(
    ...
    _check_hop_factor("corrupt_phase", hop_factor, frame_length)
    waveform = audio_tree.waveform
    ...
    keep = ("lufs", "lufs_windows") if keep_lufs else ()
    return audio_tree._invalidate_derived(keep=keep, waveform=waveform)
...
def _corrupt_phase_np(
    ...
    keep = ("lufs", "lufs_windows") if keep_lufs else ()
    return audio_tree._invalidate_derived(keep=keep, waveform=result)
...
def _shift_phase_jax(
    ...
    _check_hop_factor("shift_phase", hop_factor, frame_length)
    waveform = audio_tree.waveform
    B, C, length = waveform.shape
...
def _invalidate_offset(metadata: dict) -> dict:
    ...
    ``None``. A metadata dict without an ``offset`` key is returned unchanged.
...
def _trim_jax(audio_tree: AudioTree, length: float, mode: str = "wrap") -> AudioTree:
    ...
    # Changing the audio length invalidates every derived field.
    return audio_tree._invalidate_derived(waveform=waveform)
