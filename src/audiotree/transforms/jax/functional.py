...
def volume_norm(
    ...
    """
    # A cached ``lufs`` is trusted, the same way ``normalize_lufs`` trusts it:
    # every operation that changes the audio clears the cache.
...
def resample(
    ...
    ``engine`` is passed to :meth:`~audiotree.core.AudioTree.resample`. Inside
    ``jax.jit`` only ``"jax"`` (or the default) can run; ``"soxr"`` raises.
    """
...
def rescale_audio(audio_tree: AudioTree) -> AudioTree:
    ...
    return _rescale_audio_jax(audio_tree)
...
def peak_norm(audio_tree: AudioTree) -> AudioTree:
    ...
    """
    return _peak_norm_jax(audio_tree)
