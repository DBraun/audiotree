...
class AudioCodec(Protocol):
    ...
    ``codes``: an integer array whose shape convention (e.g. ``(batch,
    codebooks, frames)``) is defined by the codec.
    ...
    mono encoder). Any state it needs beyond the codes -- for example a
    loudness-normalization factor applied before quantizing -- is the codec's
    own to keep; audiotree stores only what ``encode`` returns.
    ...
    def encode(self, audio: AudioTree) -> ArrayLike: ...
...
class _EncodeWithCodec(_CodecTransform):
    def _apply_transform(self, audio_tree: AudioTree) -> AudioTree:
        if audio_tree.codes is not None:
            return audio_tree
        codes = self.codec.encode(audio_tree)
        return audio_tree.replace(codes=codes)
...
def encode_with_codec(
    ...
    *,
    ...
    the shape convention). AudioTrees that already have ``codes`` pass through
    unchanged.

    Args:
        codec: Object implementing ``encode(AudioTree) -> codes``
