...
The method signature is the whole contract, but it leaves three
responsibilities on the codec's side of the line:
...
**The codec owns** ``scale``. It is the second element of ``encode``'s return
...
.. testcode::
    ...
    class ToyCodec:
        ...
        def encode(self, audio: AudioTree):
            ...
            # 2. Peak-normalize, and keep the factor so a decoder can undo it.
            ...
            pooled = normalized[..., : frames * self.hop_length].reshape(
                batch, 1, frames, self.hop_length
            ).mean(axis=-1)                                    # (B, 1, frames)
            codes = np.clip(
...
.. testcode::
    ...
    codec = ToyCodec()
    tree = AudioTree(np.full((4, 2, 44_100), 0.25, dtype=np.float32), 44_100)
...
.. testcode::
    ...
    encoded = encode_with_codec(ToyCodec()).map(tree)
...
.. testcode::
    ...
    already = encode_with_codec(ToyCodec()).map(tree)
...
.. testcode::
    ...
    result = encode_latents(
        ToyCodec(), scope=["dry"], output_key="dry_latents"
    ).map({"dry": tree})
...
.. testcode::
    ...
    class JitCodec:
        ...
        def encode(self, audio: AudioTree):
            ...
            pooled = waveform[..., : frames * self.hop_length].reshape(
                batch, 1, frames, self.hop_length
            ).mean(axis=-1)
            return jnp.round(pooled * 127).astype(jnp.int32), None
