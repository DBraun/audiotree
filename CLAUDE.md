...
1. **Writer (`audiotree.writer`)**: The `AudioWriter` class provides sequential writing of AudioTree batches to disk as individual audio files plus an NPZ manifest with one row per item (loudness, pitch, the dedicated `filepath`/`source` bookkeeping columns, custom tags, and `extras_*` columns that are purely user payload). `write_audio=False` writes the manifest alone (e.g. embeddings/features in `extras`). Its output is read back with `audiotree.sources.AudioDataSource` (a Grain `RandomAccessDataSource`) or `AudioTree.from_manifest()` (the whole manifest as one batched AudioTree, with an optional `filter_fn`); both restore the recorded `extras` arrays and the `filepath`/`source` provenance.
The `TreeWriter` class (`audiotree.tree_writer`) writes arbitrary pytrees — AudioTrees, dicts of AudioTrees, or nested structures — to memory-mapped binary files (with `bagz` for string leaves), enabling zero-copy random access via `audiotree.sources.TreeDataSource` as a Grain `RandomAccessDataSource`. An AudioTree node serializes both per-item dicts as separate children: a `metadata` node holding only the `filepath`/`source` leaves, and an `extras` node holding the user's leaves.

Three "metadata"-adjacent concepts stay distinct — keep them straight in docs and code: `AudioTree.extras` is the *user's* per-item payload dict (the library plants only `offset`, `read_error`, and `codec_scale` there); `AudioTree.metadata` is the *library-internal* per-item provenance container (only the encoded `filepath`/`source`; closed schema — anything else in a stored `metadata` node is rejected by name at read); `TreeWriter(metadata=...)` is *dataset-level* metadata, stored once under the manifest's top-level `"metadata"` key and read via `TreeDataSource.get_metadata()`.
