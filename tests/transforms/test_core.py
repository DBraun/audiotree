...
class _FakeCodec:
    ...
    def __init__(self, num_codebooks: int = 4, frame_rate: int = 100):
    ...
    def encode(self, audio_tree: AudioTree):
        ...
        return jnp.zeros((B, self.num_codebooks, frames), dtype=jnp.int32)
...
def test_encode_with_codec_stores_codes():
    B = 2
...
def test_roll_invalidates_offset():
    ...
    audio_tree = AudioTree.create(waveform, 10000, offset=5.0)
    rng = np.random.default_rng(0)
    ...
    assert "offset" not in rolled._metadata
