...
def test_default_excerpt_varies_the_offset(tmp_path):
    ...
    def offsets(**kwargs):
        ...
        return [float(ds[i].offset[0]) for i in range(6)]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (dict(strategy="start", search="bias_early"), "strategy='start'"),
        (dict(strategy="loudest", search="nope"), "Unknown search"),
        (dict(search="nope"), "Unknown search"),
    ],
)
...
def test_excerpt_config_search_applies_to_random():
    """``search`` drives the single draw of strategy='random', not just 'loudest'.

    ``AudioTree.excerpt`` routes the random strategy's one offset through
    ``resolved_search``, so a custom offset distribution is expressible there.
    """
    config = ExcerptConfig(search="bias_early")
    assert config.strategy == "random"
    assert callable(config.resolved_search)
