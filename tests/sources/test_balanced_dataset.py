...
from audiotree.sources.core import AudioReadError, _derive_group_seed
...
class TestBalancedDatasetValidation:
    ...
    def test_on_read_error_reaches_every_file_based_group(self):
        """One corrupt file per group is dropped, and the mix still batches.

        The policy is threaded into every file-based group's loader, so a
        corrupt file in either group comes back as ``None`` instead of ending
        the run, and the surviving items collate normally.
        """
        with tempfile.TemporaryDirectory() as tmpdir:
            a_dir = _create_test_audio_files(tmpdir, "a", 2)
            b_dir = _create_test_audio_files(tmpdir, "b", 2)
            ...
            # third of the mix is dropped -- from both groups.
            assert sum(item is None for item in items) == 4

            real = [item for item in items if item is not None]
            batch = AudioTree.batch(real)
            assert batch.waveform.shape == (8, 1, 22050)
            assert set(batch.source) == {"a", "b"}
            # Every surviving item is real audio, never fabricated silence.
            for waveform in batch.waveform:
                assert bool(np.any(np.asarray(waveform)))
