...
from audiotree.sources.core import AudioReadError
...
def test_constructor_accepts_directory_or_manifest_path():
    """A dataset directory and its manifest path open the same dataset."""
    with tempfile.TemporaryDirectory() as tmpdir:
        output_dir = Path(tmpdir)
        ...
        # A directory locates its manifest.npz, mirroring TreeDataSource...
        source = AudioDataSource(output_dir)
        assert len(source) == 2
        assert source[0].waveform.shape == (1, 1, 8000)

        # ...and an explicit manifest file path works identically.
        by_manifest = AudioDataSource(output_dir / "manifest.npz")
...
if __name__ == "__main__":
    ...
    test_filter_by_tag()
    test_filter_by_lufs()
    test_constructor_accepts_directory_or_manifest_path()
...
def test_array_fields_keep_their_batch_axis_and_dtype():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        source = AudioDataSource(str(output_dir))
        item = source[0]
...
def test_filters_compose_with_constructor_filter_and_num_records():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        capped = AudioDataSource(output_dir, num_records=2)
        assert len(capped) == 2
...
def test_manifest_is_read_without_unpickling(tmp_path, monkeypatch):
    ...
    source = AudioDataSource(tmp_path)
    assert len(source) == 2
...
def test_getitem_drops_unreadable_items(tmp_path, breakage, policy):
    """A non-raising policy returns None, grain's filtered-element sentinel."""
    output_dir = _writer_output_with_one_bad_file(tmp_path, BREAKAGES[breakage])
    source = AudioDataSource(output_dir, on_read_error=policy)
    ...
    assert item is None
    # The neighboring entries still load normally.
    assert source[0].waveform.shape == (1, 2, 8000)
...
def test_getitem_warn_names_the_file_and_skip_stays_quiet(tmp_path):
    ...
    warning_source = AudioDataSource(output_dir, on_read_error="warn")
    with pytest.warns(UserWarning, match="audio_0001.wav"):
        warning_source[1]

    quiet_source = AudioDataSource(output_dir, on_read_error="skip")
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        quiet_source[1]
    # librosa warns on its own when it falls back to audioread; what must be
    # absent is *our* drop notice.
    assert not [w for w in caught if "Skipping unreadable" in str(w.message)]


def test_real_items_batch_after_drops(tmp_path):
    """The surviving items collate normally once the Nones are filtered out."""
    output_dir = _writer_output_with_one_bad_file(tmp_path, _empty)
    source = AudioDataSource(output_dir, on_read_error="skip")

    items = [source[i] for i in range(len(source))]
    assert [item is None for item in items] == [False, True, False]
    batch = AudioTree.batch([item for item in items if item is not None])
    assert batch.waveform.shape == (2, 2, 8000)


def test_drops_preserve_real_item_provenance(tmp_path):
    """Dropping the broken entry leaves the surviving items' provenance intact."""
    tree = AudioTree.create(
    ...
    source = AudioDataSource(tmp_path, on_read_error="skip")
    items = [source[i] for i in range(3)]

    assert items[1] is None
    real = [item for item in items if item is not None]
    assert sorted(real[0]._metadata) == ["filepath", "offset", "source"]

    batch = AudioTree.batch(real)
    assert batch.filepath == ["origin_0.wav", "origin_2.wav"]
    assert batch.source == ["music", "music"]


def test_extras_stay_empty_under_every_policy(tmp_path):
    """No policy plants bookkeeping in the user's extras dict."""
    with AudioWriter(tmp_path) as writer:
        writer.write(AudioTree.create(np.zeros((2, 1, 8000), dtype=np.float32), 8000))

    for policy in ("raise", "warn", "skip"):
        source = AudioDataSource(tmp_path, on_read_error=policy)
        assert source[0].extras == {}
...
def test_unknown_on_read_error_is_rejected_at_construction(tmp_path):
    ...
    with pytest.raises(ValueError, match="on_read_error must be one of"):
        AudioDataSource(tmp_path, on_read_error="ignore")
...
def test_drops_collate_under_resampling(tmp_path):
    """Dropping the broken entry leaves a resampled batch of real items."""
    output_dir = _writer_output_with_one_bad_file(
        tmp_path, _empty
    )  # 3x (2, 8000) @ 8 kHz
    source = AudioDataSource(output_dir, sample_rate=16000, on_read_error="skip")
    ...
    assert items[1] is None
    real = [item for item in items if item is not None]
    assert all(item.waveform.shape == (1, 2, 16000) for item in real)
    assert AudioTree.batch(real).waveform.shape == (2, 2, 16000)


def test_noninteger_ratio_batching_after_drops(tmp_path):
    """Real items keep librosa's exact resample length; the drop changes nothing.
    ...
    ratio); the surviving items must still agree with each other.
    """
    tree = AudioTree.create(
    ...
    source = AudioDataSource(tmp_path, sample_rate=48000, on_read_error="skip")
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        items = [source[i] for i in range(len(source))]
    ...
    assert items[1] is None
    real = [item for item in items if item is not None]
    assert AudioTree.batch(real).waveform.shape == (2, 1, 48001)
...
def test_token_only_manifest_honors_mono_and_resampling(tmp_path):
    ...
    source = AudioDataSource(tmp_path, sample_rate=16000, mono=True)
    item = source[0]
    ...
    dur_source = AudioDataSource(tmp_path, sample_rate=16000, mono=True, duration=0.5)
