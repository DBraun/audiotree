...
def test_audiotree_create_with_filepaths():
    ...
    assert "filepath" in tree1._metadata
    ...
    assert "filepath" not in tree4._metadata
...
def test_audiotree_create_with_source():
    ...
    assert "source" in tree1._metadata
    assert tree1.extras == {}
    ...
    assert tree3.source == []
    assert "source" not in tree3._metadata
...
def test_provenance_decodes_at_rank_4():
    ...
    mini = tree.reshape_mini_batches(2)
    assert mini._metadata["filepath"].shape == (2, 2, 1024)
...
def test_provenance_without_leading_axis_raises():
    ...
    tree = AudioTree(np.zeros((1, 1, 16)), 16000, _metadata={"filepath": encoded})
...
def test_create_rejects_provenance_list_of_wrong_length():
    ...
    assert bc._metadata["filepath"].shape[0] == 4
...
def test_audiotree_create_extras_handling():
    ...
    assert "filepath" in tree2._metadata
    assert tree2.extras == {}
    assert tree2.filepath == ["test2.wav"]
...
def test_clear_lufs_drops_both_cached_fields():
    """``clear_lufs`` is the documented shorthand for the manual invalidation."""
    audio = AudioTree(np.full((2, 1, 16_000), 0.1, np.float32), 16_000).replace_lufs()
    assert audio.lufs is not None and audio.lufs_windows is not None

    cleared = audio.clear_lufs()
    assert cleared.lufs is None
    assert cleared.lufs_windows is None
    # The original is untouched, and everything else carries over.
    assert audio.lufs is not None
    np.testing.assert_array_equal(cleared.waveform, audio.waveform)
...
def test_create_broadcasts_a_single_name_over_the_batch(key):
    ...
    assert tree._metadata[key].shape[0] == 4
    ...
    assert single._metadata[key].shape[0] == 1
...
def test_invalidate_derived_rejects_unknown_keep():
    ...
    from audiotree.core import DERIVED_FIELDS
...
def test_from_manifest_and_audio_data_source_agree(tmp_path):
    ...
    # AudioDataSource routes each row through ``from_file``, which stamps the
    # read offset into metadata; the manifest recorded none, so from_manifest
    # leaves it unset. Extras agree exactly -- they are purely user payload.
    assert per_item.extras.keys() == combined.extras.keys()
    assert combined.offset is None
    np.testing.assert_allclose(per_item.offset, [0.0, 0.0, 0.0])
    np.testing.assert_array_equal(
