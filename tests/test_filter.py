...
def test_filter_basic():
    ...
    def keep_even_batches(mini_tree):
        ...
        first_sample = mini_tree.audio_data[0, 0, 0]  # Get first sample value
    ...
    assert filtered_tree.audio_data.shape == (2, 1, 3)
    # First filtered batch should be original batch 0
    np.testing.assert_array_equal(filtered_tree.audio_data[0], [[1.0, 2.0, 3.0]])
    # Second filtered batch should be original batch 2
    np.testing.assert_array_equal(filtered_tree.audio_data[1], [[7.0, 8.0, 9.0]])
...
def test_filter_single_item():
    ...
    def keep_second_batch(mini_tree):
        first_sample = mini_tree.audio_data[0, 0, 0]
        return first_sample == 4.0

    filtered_tree = tree.filter(keep_second_batch)

    # Should have 1 batch remaining
    assert filtered_tree.audio_data.shape == (1, 1, 3)
    np.testing.assert_array_equal(filtered_tree.audio_data[0], [[4.0, 5.0, 6.0]])
...
def test_filter_keep_all():
    ...
    assert filtered_tree.audio_data.shape == (3, 1, 3)
    np.testing.assert_array_equal(filtered_tree.audio_data, audio_data)
...
def test_filter_preserves_sample_rate():
    ...
    def keep_first(mini_tree):
        return mini_tree.audio_data[0, 0, 0] == 1.0
