...
def test_source_column_round_trips_through_both_readers(tmp_path):
    ...
    ds = AudioDataSource(tmp_path)
    assert [ds[i].source for i in range(3)] == [["music"], ["speech"], ["music"]]
    assert ds[2].filepath == ["in_2.wav"]
    # Extras is untouched by provenance; the offset lives in metadata.
    assert sorted(ds[0].extras) == []
    np.testing.assert_allclose(ds[0].offset, [0.0])
