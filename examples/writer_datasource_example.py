...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Load all files (a directory locates its manifest.npz)
        ...
        # The AudioTree's .filepath reports the *recorded source* path (the
        # one written into the manifest), not the on-disk output file, which
        # the manifest entry lists as 'filename'. Tags live on the entry too.
        print(f"    - Source filepath: {first_audio.filepath[0]}")
        print(f"    - On-disk file: {source.get_entry(0)['filename']}")
